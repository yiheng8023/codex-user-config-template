...
本模板是模块化资源治理体系中的一条 lane：

- `resource-radar` 负责发现和评估公开资源。
- `agent-skills-curated` 负责已审查 Skill 正文和 release manifest。
- 书签仓负责公开安全来源目录与私有 overlay。
...
config/                  占位示例配置
docs/                    公开/私有边界与搭建说明
hooks/                   Hook 策略占位，不是 live 自动化
memory/                  记忆边界占位，不是真实记忆
scripts/verify.py        公开安全与结构验证
skills/                  Skill 安装策略占位，不 vendoring Skills
```
