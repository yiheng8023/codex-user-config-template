...
Agent 负责当前任务的连续性和能力编排。顺序工作应留在已绑定的 checkout 中；只有宿主
限制或范围边界要求切换时，才进行经过验证的交接。能力暴露随需求开始，也应随需求结束。
...
私有 bootstrap 默认应只安装薄内核。第一方协作 Skills 只有在复现残余缺口后，才能通过
显式 opt-in 投影。重新运行默认 bootstrap 不等于解除投影；负责其生命周期的机制应主动
移除已有投影，并验证最终状态。
